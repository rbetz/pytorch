...
bool available(
  ...
  COUT_FLF0;
  // XXX Conditions for vulkan convolution, synced with Convolution.cpp
  // use_vulkan
...
// XXX For Vulkan we use only prepacked weights that can be used for different
// convolution configurations(different padding, stride, dilation).
// Do we need to store padding, stride, dilation or just weights and use it for
// different padding,stride,dilation groups?
ContextConv2D create(
  ...
  return ContextConv2D{at::native::vulkan_convolution_prepack_weights(weight),
                       bias.has_value()
                           ? c10::optional<at::Tensor>((*bias).vulkan())
                           : c10::nullopt,
                       {weight_nchw.sizes()[0],
                        weight_nchw.sizes()[1],
                        weight_nchw.sizes()[2],
